...
export class AuthController {
    ...
    @Post('login')
    async login(@Body() dto: LoginDto, @Res() response: Response, @UserAgent() agent: string) {
        const tokens = await this.authService.login(dto, agent);
        if (!tokens) {
            throw new BadRequestException(`Не получается войти ${dto}`);
        }
        this.setRefreshTokenToCoockies(tokens, response);
    ...
    private setRefreshTokenToCoockies(tokens: Tokens, response: Response) {
        if (!tokens) {
            throw new UnauthorizedException();
        }
